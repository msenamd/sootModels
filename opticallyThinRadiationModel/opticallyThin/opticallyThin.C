...
) const
{
    ...
    volScalarField fv_ = mesh_.objectRegistry::template lookupObject<volScalarField>("fv");
   
    const dimensionedScalar C("C", dimensionSet(1,-1,-3,-5,0,0,0), 3.334e-4);    

    return fvm::Su
    (
        -Rp()*( T4 - pow4(Tb_)) - C*fv_*T_*T4,
        he        
    );
}
// ************************************************************************* //
